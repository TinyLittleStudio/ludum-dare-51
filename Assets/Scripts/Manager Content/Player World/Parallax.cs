...
namespace TinyLittleStudio.LudumDare51.PROJECT_NAME
{
    public class Parallax
    {
        ...
        public void InvokeCreate()
        {
            ...
            {
                parallaxObjects = new ParallaxObject[Parallax.LAYER_COUNT, Parallax.LAYER_DEPTH];

                for (int i = 0; i < Parallax.LAYER_COUNT; i++)
                {
                    for (int j = 0; j < Parallax.LAYER_DEPTH; j++)
                    {
                        if (GameObjectUtils.Instantiate<ParallaxObject>($"Prefabs/Content/Player World/LAYER_{j}", out ParallaxObject parallaxObject))
                        {
            ...
            if (parallaxObjects != null)
            {
                for (int i = 0; i < Parallax.LAYER_COUNT; i++)
                {
                    for (int j = 0; j < Parallax.LAYER_DEPTH; j++)
                    {
                        ...
                        {
                            GameObject.Destroy(parallaxObject);
                        }
                    }
                }
            }
